...
def main() -> None:
    ...
    # Preserve the first-appearance species order in the source table.
    # This order is part of the fixed randomization stream used for the
    # manuscript-associated run (D. melanogaster, D. yakuba, D. virilis).
    species_order = list(dict.fromkeys(species.tolist()))
    species_indices = [
        np.where(species == sp)[0] for sp in species_order
    ]
